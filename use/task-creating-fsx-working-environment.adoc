...
* Der ARN einer IAM-Rolle, die Workload Factory die Berechtigungen gibt, die zum Erstellen eines FSX für ONTAP-Filesystems erforderlich sind. link:task-setting-up-permissions-fsx.html["Erfahren Sie, wie Sie einem AWS-Konto Berechtigungen erteilen"^].
